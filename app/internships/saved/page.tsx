...
export default function SavedInternshipsPage() {
  ...
  useEffect(() => {
    // Get saved internship IDs from localStorage
    const savedIds = JSON.parse(localStorage.getItem('savedInternships') || '[]')
    // Get all internships and filter for saved ones
    const allInternships = getAllInternships()
    const saved = allInternships.filter(internship => savedIds.includes(internship.id))
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {savedInternships.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-2">No saved internships</h2>
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <button 
                  onClick={() => {
                    const savedIds = JSON.parse(localStorage.getItem('savedInternships') || '[]')
                    const newSavedIds = savedIds.filter((id: string) => id !== internship.id)
                    localStorage.setItem('savedInternships', JSON.stringify(newSavedIds))
                    setSavedInternships(current => 
                      current.filter(saved => saved.id !== internship.id)
                    )
                  }}
